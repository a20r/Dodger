...
int main() {
    ...
    double speeds [num_speeds] = {4, 4.5};

    const int num_noises = 1;
    double noises [num_noises] = {0.004};

    const int num_wait_times = 5;
    double wait_times [num_wait_times] = {0.1, 0.3, 0.5, 0.7, 0.9};
    ...
    for (int i = 0; i < num_speeds; i++) {
        for (int j = 0; j < num_noises; j++) {
            for (int l = 0; l < num_wait_times; l++) {
                double wait_time = wait_times[l];
